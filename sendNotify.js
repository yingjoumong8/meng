...
// PUSH_SCRIPT_NAME推送脚本名称 逗号分割，不在就不推
let PUSH_SCRIPT_NAME = process.env.PUSH_SCRIPT_NAME && process.env.PUSH_SCRIPT_NAME.split('@') || [];
...
async function sendNotify(text, desp, params = {TG交流群:'https://t.me/jd_zero_205'}, author = 'zero205') {
  ...
  if(PUSH_SCRIPT_NAME.includes(text)){
	  await Promise.all([
